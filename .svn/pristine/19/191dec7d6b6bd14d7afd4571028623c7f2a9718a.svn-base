...
trait SortDao {
  ...
  lazy val ddl = SortJob.ddl ++ SortRddHistory.ddl ++ SortTask.ddl
   ...
   *  @param sortFields Database column sort_fields DBType(TINYTEXT), Length(255,true)
   *  @param filterFields Database column filter_fields DBType(TINYTEXT), Length(255,true)
   *  @param key Database column key DBType(TINYTEXT), Length(255,true)
   ...
   *  @param extFeature Database column ext_feature DBType(TEXT), Length(65535,true), Default(None) */
  ...
  class SortJob(_tableTag: Tag) extends Table[SortJobRow](_tableTag, "sort_job") {
    ...
    /** Database column sort_fields DBType(TINYTEXT), Length(255,true) */
    val sortFields: Column[String] = column[String]("sort_fields", O.Length(255,varying=true))
    /** Database column filter_fields DBType(TINYTEXT), Length(255,true) */
    val filterFields: Column[String] = column[String]("filter_fields", O.Length(255,varying=true))
    /** Database column key DBType(TINYTEXT), Length(255,true) */
    val key: Column[String] = column[String]("key", O.Length(255,varying=true))
    ...
    /** Database column ext_feature DBType(TEXT), Length(65535,true), Default(None) */
    val extFeature: Column[Option[String]] = column[Option[String]]("ext_feature", O.Length(65535,varying=true), O.Default(None))
  }
  /** Collection-like TableQuery object for table SortJob */
  lazy val SortJob = new TableQuery(tag => new SortJob(tag))
  
  /** Entity class storing rows of table SortRddHistory
   *  @param id Database column id DBType(INT), AutoInc, PrimaryKey
   *  @param taskId Database column task_id DBType(INT), Default(0)
   *  @param rddPath Database column rdd_path DBType(TEXT), Length(65535,true)
   *  @param isOutput Database column is_output DBType(TINYINT)
   *  @param isFull Database column is_full DBType(TINYINT)
   *  @param createdOn Database column created_on DBType(TIMESTAMP)
   *  @param updatedOn Database column updated_on DBType(TIMESTAMP) */
  case class SortRddHistoryRow(id: Int, taskId: Int = 0, rddPath: String, isOutput: Byte, isFull: Byte, createdOn: java.sql.Timestamp, updatedOn: java.sql.Timestamp)
  /** GetResult implicit for fetching SortRddHistoryRow objects using plain SQL queries */
  implicit def GetResultSortRddHistoryRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Byte], e3: GR[java.sql.Timestamp]): GR[SortRddHistoryRow] = GR{
    prs => import prs._
    SortRddHistoryRow.tupled((<<[Int], <<[Int], <<[String], <<[Byte], <<[Byte], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  /** Table description of table sort_rdd_history. Objects of this class serve as prototypes for rows in queries. */
  class SortRddHistory(_tableTag: Tag) extends Table[SortRddHistoryRow](_tableTag, "sort_rdd_history") {
    def * = (id, taskId, rddPath, isOutput, isFull, createdOn, updatedOn) <> (SortRddHistoryRow.tupled, SortRddHistoryRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (id.?, taskId.?, rddPath.?, isOutput.?, isFull.?, createdOn.?, updatedOn.?).shaped.<>({r=>import r._; _1.map(_=> SortRddHistoryRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    
    /** Database column id DBType(INT), AutoInc, PrimaryKey */
    val id: Column[Int] = column[Int]("id", O.AutoInc, O.PrimaryKey)
    /** Database column task_id DBType(INT), Default(0) */
    val taskId: Column[Int] = column[Int]("task_id", O.Default(0))
    /** Database column rdd_path DBType(TEXT), Length(65535,true) */
    val rddPath: Column[String] = column[String]("rdd_path", O.Length(65535,varying=true))
    /** Database column is_output DBType(TINYINT) */
    val isOutput: Column[Byte] = column[Byte]("is_output")
    /** Database column is_full DBType(TINYINT) */
    val isFull: Column[Byte] = column[Byte]("is_full")
    /** Database column created_on DBType(TIMESTAMP) */
    val createdOn: Column[java.sql.Timestamp] = column[java.sql.Timestamp]("created_on")
    /** Database column updated_on DBType(TIMESTAMP) */
    val updatedOn: Column[java.sql.Timestamp] = column[java.sql.Timestamp]("updated_on")
    
    /** Index over (taskId) (database name task_id) */
    val index1 = index("task_id", taskId)
  }
  /** Collection-like TableQuery object for table SortRddHistory */
  lazy val SortRddHistory = new TableQuery(tag => new SortRddHistory(tag))
